...
# This function displays the home page with categories and newly added 8 categories. 
@app.route('/')
def showCategory():
        if checkCookie():
...
def gconnectOld():
    ...
    return "username" + login_session['username'] + ":" + login_session['picture'] + "<br>" + login_session['email'] 
    user_id = getUserId(login_session['email'])
    if  not user_id:
            user_id = createUser(login_session)
            strUser = "New User"
...
# This method calls google server to disconnect the logged in user with google id
